...
use crate::enums::{Bmi3Intf, Bmi3SensConfigTypes, Bmi3Error};
// use core::usize;
...
pub struct Bmi3Dev {
    ...
    pub read: Option<fn(reg_addr: u8, reg_data: &mut [u8], length: u32, intf_ptr: *mut ()) -> I2cResult<()>>,
...
impl Bmi3Dev {
    ...
    pub fn bmi3_init(&mut self) -> Bmi3Result<()> {
        ...
        self.accel_bit_width = if ((chip_id[1] & BMI3_REV_ID_MASK) >> BMI3_REV_ID_POS) == BMI3_ENABLE {
        ...
        if self.chip_id as u16 == BMI323_CHIP_ID {
    ...
    pub fn bmi3_map_interrupt(&mut self, map_int: &mut Bmi3MapInt) -> Bmi3Result<()> {
        ...
        set_bits!(reg_data, 1, 8,
            (BMI3_STEP_DETECTOR_OUT_MASK, map_int.step_detector_out as u16, Some(BMI3_STEP_DETECTOR_OUT_POS)),
            (BMI3_STEP_COUNTER_OUT_MASK, map_int.step_counter_out as u16, Some(BMI3_STEP_COUNTER_OUT_POS)),
            (BMI3_SIG_MOTION_OUT_MASK, map_int.sig_motion_out as u16, Some(BMI3_SIG_MOTION_OUT_POS)),
            (BMI3_TILT_OUT_MASK, map_int.tilt_out as u16, Some(BMI3_TILT_OUT_POS))
        );
        set_bits!(reg_data, 2,
            (BMI3_TAP_OUT_MASK, map_int.tap_out as u16, None::<u8>),
            (BMI3_I3C_OUT_MASK, map_int.i3c_out as u16, Some(BMI3_I3C_OUT_POS)),
            (BMI3_ERR_STATUS_MASK, map_int.err_status as u16, Some(BMI3_ERR_STATUS_POS)),
            (BMI3_TEMP_DRDY_INT_MASK, map_int.temp_drdy_int as u16, Some(BMI3_TEMP_DRDY_INT_POS))
        );
        set_bits!(reg_data, 3, 8,
            (BMI3_GYR_DRDY_INT_MASK, map_int.gyr_drdy_int as u16, Some(BMI3_GYR_DRDY_INT_POS)),
            (BMI3_ACC_DRDY_INT_MASK, map_int.acc_drdy_int as u16, Some(BMI3_ACC_DRDY_INT_POS)),
            (BMI3_FIFO_WATERMARK_INT_MASK, map_int.fifo_watermark_int as u16, Some(BMI3_FIFO_WATERMARK_INT_POS)),
            (BMI3_FIFO_FULL_INT_MASK, map_int.fifo_full_int as u16, Some(BMI3_FIFO_FULL_INT_POS))
        );
    ...
    pub fn get_interrupt_status(&mut self, int_status: &mut u16, register: u8) -> Bmi3Result<()> {
        ...
        self.bmi3_get_regs(register, &mut data_array, 2)
            .map(|()| {
